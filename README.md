...
4. **分辨率声明（可选）**：如果脚本里没有 `RES w h`，默认 **脚本分辨率=设备分辨率**（不做缩放）。如果脚本是按 1280×720 编写，建议在脚本第一行写 `RES 1280 720`，这样设备分辨率不同也会自动换算坐标后再点击/滑动。
...
| `RES w h` | （可选）声明脚本坐标系分辨率；缺省时默认与设备一致。声明后 `CLICK x y` 与 `SWIP`（脚本坐标变量）会自动按设备分辨率缩放 |
